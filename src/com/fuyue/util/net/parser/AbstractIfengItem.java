...
/**
 * 自动解析抽象类，接口数据item如需自动解析，需要继承自对应解析格式的子类:{@link AbstractIfengJSONItem}提供 JSON解析、
 * {@link AbstractIfengXMLItem}提供XML解析。具体的解析路径编写参照具体类目描述。 注意：
 * {@link #AbstractIfengItem}可作为外部类和静态内部类，不能为非静态的内部类。
 * 
 * @author Calvin 2013-5-31
 * 
 */
public abstract class AbstractIfengItem implements Serializable {
	...
	/**
	 * 根据class获取实例,Class要求继承自{@link AbstractIfengItem}
	 * 
	 * @param classs
	 * @return
	 */
	public static AbstractIfengItem getInstance(
			Class<? extends AbstractIfengItem> classs) {
		try {
			Constructor<? extends AbstractIfengItem> constructor = null;
