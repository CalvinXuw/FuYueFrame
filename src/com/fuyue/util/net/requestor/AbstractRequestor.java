...
import com.fuyue.util.net.requestor.RequestTask.OnRequestTaskListener;
...
public abstract class AbstractRequestor extends AbstractModel {
	...
	/** 自动解析的Class类型 */
	private Class<? extends AbstractIfengItem> mAutoParseClass;
	...
	protected void process() {
		...
		if (canUseCache() && mRequestDataCache.isEffective()) {
			mRequestTask = new CacheRequestTask(mRequestDataCache,
					mCacheOnRequestTaskListener);
		} else {
			mRequestTask = new WebRequestTask(mContext, url,
					getRequestHeaders(), getCombineParams(), getPriority(),
					mWebOnRequestTaskListener);
			((WebRequestTask) mRequestTask).setRequestType(mRequestType);
			ifNeedPreloadFromCache();
		}
	...
	private synchronized void parseResult(String data, boolean isFromPreload) {
		AbstractIfengItem item = null;
		/*
		 * 如果可以进行自动解析
		 */
		if (canUseAutoParse()) {
			item = AbstractIfengItem.getInstance(mAutoParseClass);
			/*
			 * 如果成功自动解析
			 */
	 ...
	 * @return
	 */
	private List<NameValuePair> getCombineParams() {
	 ...
	 * @param item
	 */
	protected abstract void handleResult(AbstractIfengItem item);
	 ...
	 * @param item
	 */
	protected void handlePreloadResult(AbstractIfengItem item) {
		/*
	 ...
	 *            result
	 */
	protected AbstractIfengJSONItem handleUnparseResult(String result) {
		/*
	 ...
	 * 子类中可自行重写实现所需的缓存策略。
	 */
	protected void fillCacheFileTag() {
	 ...
	 * 关闭cache预加载,默认为关闭状态
	 */
	private void turnOffPreloadFromCache() {
		mNeedPreload = false;
	 ...
	 * 从缓存中预加载
	 */
	private void ifNeedPreloadFromCache() {
		if (mNeedPreload && canUseCache()) {
			if (mRequestDataCache.isExist()) {
				new CacheRequestTask(mRequestDataCache,
						mPreloadOnRequestTaskListener).execute();
			}
		}
	}
	 ...
	 */
	public void setAutoParseClass(
			Class<? extends AbstractIfengItem> autoParseClass) {
