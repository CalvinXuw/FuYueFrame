...
public class DownloadManager {
	 ...
	 * completes.
	 */
	public static final String ACTION_DOWNLOAD_COMPLETE = Constants.PACKAGENAME
	 ...
	 * downloads UI.
	 */
	public static final String ACTION_NOTIFICATION_CLICKED = Constants.PACKAGENAME
			+ ".intent.action.DOWNLOAD_NOTIFICATION_CLICKED";

	/**
	 * Intent action to launch an activity to display all downloads.
	 */
	public static final String ACTION_VIEW_DOWNLOADS = Constants.PACKAGENAME
			+ ".intent.action.VIEW_DOWNLOADS";
	 ...
	 * @return
	 */
	public long enqueueAppDownloadTask(String url, String title,
			String description, boolean isVisible) {
		return enqueueDownloadTask(url, title, description, isVisible,
	 ...
	 * @return
	 */
	public long enqueueBookDownloadTask(String url, String title,
			String description, boolean isVisible) {
		return enqueueDownloadTask(url, title, description, isVisible,
				Constants.MIMETYPE_EPUB);
	}
	 ...
	 * @param isVisible
	 *            是否显示在通知栏上
	 * @return 插入数据库中的download任务ID
	 */
	private long enqueueDownloadTask(String url, String title,
			String description, boolean isVisible, String mimetype) {
		ContentValues values = new ContentValues();
		values.put(Downloads.Impl.COLUMN_URI, url);
		values.put(Downloads.Impl.COLUMN_NO_INTEGRITY, true);// 异常续传
		if (isVisible) {
			values.put(Downloads.Impl.COLUMN_VISIBILITY,
					Downloads.Impl.VISIBILITY_VISIBLE_NOTIFY_COMPLETED); // 下载仅通过notification提示进度，不提示下载完成或失败。
		} else {
			values.put(Downloads.Impl.COLUMN_VISIBILITY,
					Downloads.Impl.VISIBILITY_HIDDEN);
		}
		values.put(Downloads.Impl.COLUMN_MIME_TYPE, mimetype);
		values.put(Downloads.Impl.COLUMN_FILE_NAME_HINT, title);
		values.put(Downloads.Impl.COLUMN_IS_PUBLIC_API, false);
		values.put(Downloads.Impl.COLUMN_ALLOW_ROAMING, true);
		values.put(Downloads.Impl.COLUMN_DESCRIPTION, description); // description
		// 设置header信息,以INSERT_KEY_PREFIX+"header名"的方式作为key，header名:header值作为value
		// values.put(Downloads.Impl.RequestHeaders.INSERT_KEY_PREFIX +
		// "version",
		// "version:2013");
		values.put(Downloads.Impl.COLUMN_TITLE, title); // 下载前指定文件名，不带.后缀名
		...
		final Uri downloadUri = mResolver.insert(Downloads.Impl.CONTENT_URI,
				values);
		long id = Long.parseLong(downloadUri.getLastPathSegment());
		return id;
	}
	 ...
	 * @return 当前数据库中全部下载任务
	 */
	public List<DownloadInfo> getAllDownload(Context context) {
		SystemFacade systemFacade = new RealSystemFacade(context);
		List<DownloadInfo> downloadInfos = new LinkedList<DownloadInfo>();
		Cursor cursor = mResolver.query(mBaseUri, null, null, null, null);
		if (cursor == null) {
			return downloadInfos;
		}
		try {
			DownloadInfo.Reader reader = new DownloadInfo.Reader(mResolver,
					cursor);
	 ...
	 * @return
	 */
	public DownloadInfo getDownloadById(Context context, long... ids) {
		SystemFacade systemFacade = new RealSystemFacade(context);
		Cursor cursor = mResolver.query(mBaseUri, null,
	 ...
	 * @hide
	 */
	public void restartDownload(Context context, boolean isVisiable,
			long... ids) {
		...
		try {
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor
				...
				if (status != STATUS_SUCCESSFUL && status != STATUS_FAILED
					...
					// end 如果状态错误，直接返回
					if (Constants.LOGV) {
						Log.v(Constants.TAG, "非完成状态，不能重新发起下载。" + ids
								+ "status:" + status);
					}
					return;
				}
		...
		// 如果ids为空，表示restart全部被暂停的下载
		String where = null;
		String[] whereArgs = null;
		if ((null != ids) && (ids.length > 0)) {
			where = getWhereClauseForIds(ids);
			whereArgs = getWhereArgsForIds(ids);
		} else {
			// where：处于暂停状态的APP
			where = Downloads.Impl.COLUMN_STATUS + "=?";
			whereArgs = new String[] { String
					.valueOf(Downloads.Impl.STATUS_PAUSED_BY_APP) };
		}

		mResolver.update(mBaseUri, values, where, whereArgs);
	 ...
	 * 
	 * @param id
	 *            the IDs of the downloads to pause
	 * @param item
	 *            数据
	 */
	...
	public void pauseAllDownload() {
		...
		mResolver.update(mBaseUri, values,
				...
				new String[] { String.valueOf(Downloads.Impl.STATUS_PENDING),
	...
	public void pauseAllSilentDownload() {
		...
		mResolver.update(
				...
				new String[] { String.valueOf(Downloads.Impl.STATUS_PENDING),
						String.valueOf(Downloads.Impl.STATUS_SUCCESS),
						String.valueOf(Downloads.Impl.VISIBILITY_HIDDEN) });
	}
	 ...
	 */
	public boolean isPaused(long id) {
		Cursor cursor = mResolver.query(getDownloadUri(id), null, null, null,
				null);
		if (cursor == null) {
			return false;
		} else {
			cursor = new CursorTranslator(cursor, mBaseUri);
		}
		cursor.moveToFirst();
		return cursor.getInt(cursor
				.getColumnIndex(Downloads.Impl.COLUMN_CONTROL)) == Downloads.CONTROL_PAUSED;
	}
