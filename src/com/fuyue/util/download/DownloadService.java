...
public class DownloadService extends Service {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			synchronized (DownloadService.this) {
				try {
					...
					// notify anyone waiting on successful connection to
					// MediaService
			...
			synchronized (DownloadService.this) {
				...
				if (mMediaScannerService != null) {
					...
					try {
						...
						// notify anyone waiting on unsuccessful connection to
						// MediaService
						DownloadService.this.notifyAll();
					}
				}
	 ...
	 */
	static class MyMediaScannerConnectionClient implements
			MediaScannerConnectionClient {
	...
	public void onCreate() {
		...
		if (this.getApplicationContext() instanceof DownloadServiceCallback) {
			((DownloadServiceCallback) this.getApplicationContext())
					.onDownloadServiceCreate();
		}
		...
		mMediaScannerConnection = new MyMediaScannerConnection(this,
		...
		public void run() {
			...
			for (;;) {
				...
				Cursor cursor = getContentResolver().query(
						Downloads.Impl.ALL_DOWNLOADS_CONTENT_URI, null, null,
						null, null);
				if (cursor == null) {
					continue;
				}
				try {
					...
					for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor
						...
						if (next == 0) {
							...
							// fix APPSEARCH-53
							// 【软件搜索】【下载】外部存储空间不足时，下载失败后，偶尔出现系统通知栏中的下载进度不会及时消失
							// 由于重试的时候 stopSelf 退出的话，
							// observer无法监听，导致删除的时候无法监听到删除。
							keepService = true;
				...
				if (!mustScan) {
					for (DownloadInfo info : mDownloads.values()) {
						if (info.mDeleted
								&& TextUtils.isEmpty(info.mMediaProviderUri)) {
							mustScan = true;
							keepService = true;
							break;
						}
				...
				Intent downloadBroadcast = new Intent(
						Constants.ACTION_DOWNLOADMANAGER_UPDATE);
				mSystemFacade.sendBroadcast(downloadBroadcast);
					...
					if (info.mDeleted) {
						...
						if (TextUtils.isEmpty(info.mMediaProviderUri)) {
							if (info.shouldScanFile()) {
								...
								// file.
								Helpers.deleteFile(getContentResolver(),
										info.mId, info.mFileName,
										info.mMimeType);
							}
						} else {
							...
							getContentResolver().delete(
									Uri.parse(info.mMediaProviderUri), null,
									null);
		...
		private void bindMediaScanner() {
			synchronized (DownloadService.this) {
				if (!mMediaScannerConnecting) {
					...
					// bindService(intent, mMediaScannerConnection,
					// BIND_AUTO_CREATE);
	...
	private void updateDownload(DownloadInfo.Reader reader, DownloadInfo info,
		...
		boolean justCompleted = !Downloads.Impl.isStatusCompleted(oldStatus)
				&& Downloads.Impl.isStatusCompleted(info.mStatus);
		...
		if (justCompleted) {
			// 下载完成的intent
			Intent downloadBroadcast = new Intent(
					Constants.ACTION_DOWNLOADMANAGER_COMPLETED);
			downloadBroadcast.setData(info.getMyDownloadsUri());
			mSystemFacade.sendBroadcast(downloadBroadcast);
		}
	...
	private void deleteDownload(long id) {
		...
		if (info.mDestination != Downloads.Impl.DESTINATION_EXTERNAL
				&& info.mFileName != null) {
	 ...
	 *            whether update the database
	 * @param deleteFile
	 *            whether delete the file.
	...
	private boolean scanFile(DownloadInfo info, final boolean updateDatabase,
		...
		synchronized (this) {
			...
			if (mMediaScannerService == null) {
				// not bound to mediaservice. but if in the process of
				// connecting to it, wait until
				// connection is resolved
				while (mMediaScannerConnecting) {
					Log.d(Constants.TAG,
							"waiting for mMediaScannerService service: ");
					try {
			...
			if (mMediaScannerService == null) {
				// no available MediaService And not even in the process of
				// connecting to it
				return false;
			}
			...
			try {
				...
				OnScanCompletedListener listener = new OnScanCompletedListener() {
					...
					public void onScanCompleted(String path, Uri uri) {
						if (updateDatabase) {
							// file is scanned and mediaprovider returned uri.
							...
							if (changed) {
								int result = getContentResolver().update(key,
										values, null, null);
								if (result == 0) {
									if (Constants.LOGV) {
										Log.v(Constants.TAG,
												"Scanning file update failed "
														+ key);
									}
								}
							}
							...
							if (uri != null) {
								// use the Uri returned to delete it from the
								// MediaProvider
								getContentResolver().delete(uri, null, null);
							}
							// delete the file and delete its row from the
							// downloads db
				...
				/*
				 * 
				 * String[] paths = new String[]{info.mFileName}; String[]
				 * mimeTypes = new String[]{info.mMimeType};
				 * 
				 * MediaScannerConnection.scanFile(this, paths, mimeTypes, new
				 * OnScanCompletedListener() {
				 * 
				 * @Override public void onScanCompleted(String path, Uri uri) {
				 * if (updateDatabase) { // file is scanned and mediaprovider
				 * returned uri. store it in downloads // table (i.e., update
				 * this downloaded file's row) ContentValues values = new
				 * ContentValues(); values.put(Constants.MEDIA_SCANNED, 1); if
				 * (uri != null) {
				 * values.put(Downloads.Impl.COLUMN_MEDIAPROVIDER_URI,
				 * uri.toString()); } getContentResolver().update(key, values,
				 * null, null); } else if (deleteFile) { if (uri != null) { //
				 * use the Uri returned to delete it from the MediaProvider
				 * getContentResolver().delete(uri, null, null); } // delete the
				 * file and delete its row from the downloads db
				 * Helpers.deleteFile(resolver, id, path, mimeType); } } });
				 */
