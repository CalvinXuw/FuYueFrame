...
/**
 * 由于SDK4.0以上与SDK4.0通知的接口不同 由于notification.builder设置进度的api是在api14以上才有，
...
public abstract class VersionedNotification {
	...
	/**
	 * Supply a PendingIntent to send when the notification is cleared by the
	 * user directly from the notification panel.
	 * 
	 * @param intent
	 *            intent
	 */
	 ...
	 *            highPriority
	 */
	public abstract void setFullScreenIntent(PendingIntent intent,
	 ...
	 * 
	 * @param need
	 *            是否setLatestEventInfo
	 */
	public abstract void setLatestEventInfo(boolean need);
	...
	static final class VersionNotification13 extends VersionedNotification {
		...
		public Notification getNotification() {
			...
			if (mNeedRemove) {
				n.setLatestEventInfo(mContext, mContentTitle, mContentText,
						mContentIntent);
			}
			return n;
		}
		...
		private RemoteViews makeRemoteViews(int resId) {
			...
			if (mProgressMax != 0 || mProgressIndeterminate) {
				contentView.setProgressBar(R.id.progress_bar, mProgressMax,
						mProgress, mProgressIndeterminate);
				// contentView.setViewVisibility(R.id.progress_bar,
				// View.VISIBLE);
			...
			/*
			 * 无用代码 if (mWhen != 0) { contentView.setLong(R.id.time, "setTime",
			 * mWhen); }
			 */
			return contentView;
		}
		...
		private String getDownloadingText(long totalBytes, long currentBytes) {
			...
			long progress = currentBytes * 100 / totalBytes; // SUPPRESS
																// CHECKSTYLE
		 ...
		 * 
		 * @see
		 * com.baidu.appsearch.downloads.VersionedNotification#setLatestEventInfo
		 * (android.content.Context, java.lang.CharSequence,
		 * java.lang.CharSequence, android.app.PendingIntent)
		 */
		@Override
		public void setLatestEventInfo(boolean need) {
			this.mNeedRemove = need;
		}
	}
	...
	static final class VersionNotification14 extends VersionedNotification {
		...
		@Override
		public Notification getNotification() {
			// Modified By xuwei 2013-4-25\
			// The method getNotification() from the type Notification.Builder
			// is deprecated
			final int sdkVersion = Build.VERSION.SDK_INT;
