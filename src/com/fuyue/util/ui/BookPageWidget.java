...
public class BookPageWidget extends View {
	...
	/** 屏幕宽 */
	private int mScreenWidth = (int) Utility.getScreenWidth(getContext());
	/** 屏幕高 */
	private int mScreenHeight = (int) ((int) Utility
			.getScreenHeight(getContext()) - 50 * Utility
	...
	// private HasNextListener mHasNextListener;
	public BookPageWidget(Context context, int width, int height) {
		super(context);
		mPath0 = new Path();
		mPath1 = new Path();
		mScreenWidth = width;
	...
	/*
	 * public void setHasNextListener(HasNextListener listener) {
	 * this.mHasNextListener = listener; }
	 */
	/**
	 * 计算拖拽点对应的拖拽脚
	 * 
	...
	public boolean doTouchEvent(MotionEvent event, boolean isTouchSide) {
	...
	public void setStyle(ReadStyle style) {
		if (style == ReadStyle.DARK) {
			mBackgroundColor = 0xFF222222;
		} else {
			mBackgroundColor = 0xFFdddddd;
		}
	}
	...
	private void startAnimation(boolean needAnmi) {
		int dx, dy;
		...
		if (mCornerY > 0) {
			...
			dy = (int) (-mTouch.y) + 1; // 防止mTouch.y最终变为0，导致翻页后仍然返回前一页的bug
		}
	...
	/*
	 * public interface HasNextListener { boolean hasNext(); }
	 */
}
