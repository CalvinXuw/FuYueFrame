...
public class ImageCache {
	...
	public static ImageCache getInstance(FragmentManager fragmentManager,
		...
		final RetainFragment mRetainFragment = findOrCreateRetainFragment(
				fragmentManager, cacheParams.diskCacheDir.getPath());
	 ...
	 * @param data
	 *            Unique identifier for which item to get
	 * @return The bitmap filepath if found in cache, null otherwise
	 */
	public String getBitmapFilepathFromDiskCache(String data) {
		final String key = hashKeyForDisk(data);
			...
			if (mDiskLruCache != null) {
				...
				try {
					...
					if (snapshot != null) {
						...
						inputStream = snapshot.getInputStream(DISK_CACHE_INDEX);
						if (inputStream != null) {
							filepath = mDiskLruCache.getDirectory()
									.getAbsolutePath()
									+ File.separator
									+ key
									+ "." + DISK_CACHE_INDEX;
