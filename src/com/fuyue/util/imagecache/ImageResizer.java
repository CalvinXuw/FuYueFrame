...
package com.fuyue.util.imagecache;
...
public class ImageResizer extends ImageWorker {
	...
	@Override
	protected Bitmap processBitmap(BitmapTaskItem item,
			OnProcessProgressUpdate processCallback) {
		return processBitmap(Integer.parseInt(String.valueOf(item.mDataSource)));
	}
	 ...
	 * @return
	 */
	public static Bitmap decodeSampledBitmapFromAsset(Context context,
			String res, int reqWidth) {

		try {
			// First decode with inJustDecodeBounds=true to check dimensions
			final BitmapFactory.Options options = new BitmapFactory.Options();
			options.inJustDecodeBounds = true;
			BitmapFactory.decodeStream(context.getAssets().open(res), null,
					options);
			...
			// Decode bitmap with inSampleSize set
			options.inJustDecodeBounds = false;
			return BitmapFactory.decodeStream(context.getAssets().open(res),
					null, options);
		} catch (Exception e) {
			return null;
		}
	 ...
	 * @return
	 */
	public static Bitmap resizeBitmap(Bitmap bitmap, int framewidth,
			int frameheight) {
		...
		int minlength = framewidth > frameheight ? frameheight : framewidth;

		if (framewidth > frameheight) {
			newHeight = minlength;
			newWidth = (int) ((float) bmWidth / bmHeight * newHeight);
		} else {
			newWidth = minlength;
			newHeight = (int) ((float) bmHeight / bmWidth * newWidth);
		}
		...
		Bitmap resizedBitmap = Bitmap.createBitmap(bitmap, 0, 0, bmWidth,
	...
	public static Bitmap getRoundedCornerBitmap(Context context, Bitmap bitmap) {
		try {
			...
			final RectF rectF = new RectF(new Rect(0, 0, bitmap.getWidth(),
					bitmap.getHeight()));
			final float roundPx = 5 * Utility.getDensity(context);
